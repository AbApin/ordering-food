...
export async function POST(req) {
  ...
  for (const cartProduct of cartProducts) {
    ...
    if (cartProduct.extraIngredientPrices?.length > 0) {
      for (const cartProductExtraThing of cartProduct.extraIngredientPrices) {
  ...
  const stripeSession = await stripe.checkout.sessions.create({
    ...
    success_url: process.env.NEXTHAUTH_URL + 'orders/' + orderDoc._id.toString() + '?clear-cart=1',
    cancel_url: process.env.NEXTHAUTH_URL + 'cart?canceled=1',
